# AYAIR 2026 — Submission Checklist

## ⏰ Deadline: June 30, 2026, 21:45 GMT

---

## Step 1: Fill Personal Information

Open `registration.md` and fill in all `[FILL:]` fields:

- [ ] Date of Birth (DD/MM/YYYY format)
- [ ] Email address
- [ ] Street Address
- [ ] Postal Code
- [ ] Phone Number

---

## Step 2: Submit Jotform

Go to: **https://form.jotform.com/260073925589568**

- [ ] First Name: **Hazem**
- [ ] Last Name: **Khaled**
- [ ] Date of Birth: [your DOB]
- [ ] Email: [your email]
- [ ] Nationality: **Egyptian**
- [ ] Address: [your full address]
- [ ] Nature of Submission: **Group Project**
- [ ] Phone: [your phone with area code +20]
- [ ] Award Category: **Education Enhancement**
- [ ] Project Name: **NeuroScope**
- [ ] Project Introduction Essay: Paste the essay from `essay_final.md` (426 words)
- [ ] Complete CAPTCHA
- [ ] Click **Submit**

---

## Step 3: Save Acknowledgement

- [ ] Check your email for acknowledgement from Jotform
- [ ] Save/screenshot the confirmation

---

## Step 4: Reply with Project Paper

Reply to the acknowledgement email with:

**Subject:** NeuroScope — Project Paper — DigiNeurons Team

**Body:**
```
Dear AYAIR 2026 Team,

Please find attached the project paper for NeuroScope, submitted by the DigiNeurons team for the Education Enhancement category.

Team Members:
1. Hazem Khaled — Team Leader · Data Analyst
2. Yomna Ashraf — Data Analyst
3. Yossef Shrif — Data Analyst
4. Shahd Khairy — Frontend Developer
5. Mohamed Wagdi — Backend Developer
6. Ziad Mohamed — Backend Developer
7. Yossef Safout — Backend Developer
8. Mohamed Abdel Ghani — Data Science

Project: NeuroScope — Visual Deep Learning Builder
Category: Education Enhancement

Best regards,
Hazem Khaled
DigiNeurons Team
```

**Attach:** The project paper PDF (essay + team details + technical overview)

- [ ] Reply sent with project paper attached
- [ ] Confirmation received (or check spam folder)

---

## Step 5: Verify Submission

- [ ] Jotform submitted ✅
- [ ] Acknowledgement email saved ✅
- [ ] Project paper emailed ✅
- [ ] All team members notified ✅

---

## 📅 Key Dates to Remember

| Date | Event |
|------|-------|
| **Jun 30, 2026** | Submission deadline |
| **Sep 17, 2026** | Finalists announced |
| **Oct 12-13, 2026** | Awards ceremony (Egypt) |

---

## ⚠️ Important Rules

- **One submission per group** — only the group leader submits
- **Essay must be original** — plagiarism will disqualify you
- **Essay must be ≤800 words** — current essay is 426 words ✅
- **Group paper must list ALL team members**
- **Judges' decisions are final** — no appeals
- **Must attend ceremony if selected** — no response = forfeit prizes

---

*Last updated: 2026-06-27*
